...
const CategoryShowcase = () => {
  ...
  const handleTouchStart = (e) => { setIsDragging(false); setDragStartX(e.touches[0].clientX); };
  const handleTouchMove = (e) => { if (Math.abs(e.touches[0].clientX - dragStartX) > 5) setIsDragging(true); };
  const handleTouchEnd = () => setTimeout(() => setIsDragging(false), 100);
  const handleMouseDown = (e) => { setIsDragging(false); setDragStartX(e.clientX); };
  const handleMouseMove = (e) => { if (e.buttons === 1 && Math.abs(e.clientX - dragStartX) > 5) setIsDragging(true); };
  const handleMouseUp = () => setTimeout(() => setIsDragging(false), 100);

  const getAnimationProps = (props) => isMobile ? {} : props;
  ...
  return (
    <section className="py-16 lg:py-20 bg-gradient-to-br from-crema-50 via-white to-crema-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Carousel Container */}
        <div className="relative -mx-4 sm:mx-0 ml-1">
          <button onClick={() => scroll('left')} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-20 bg-white dark:bg-gray-800 shadow-xl hover:shadow-2xl text-morado-600 dark:text-white p-3 rounded-full transition-all duration-300 hover:scale-110 items-center justify-center" aria-label="Anterior">
            <ChevronLeft className="w-6 h-6" />
          </button>

          <button onClick={() => scroll('right')} className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-20 bg-white dark:bg-gray-800 shadow-xl hover:shadow-2xl text-morado-600 dark:text-white p-3 rounded-full transition-all duration-300 hover:scale-110 items-center justify-center" aria-label="Siguiente">
          ...
          >
            {categories.map((category, index) => (
              ...
              >
                <Link to={category.link} className="w-full text-left group">
                    ...
                    <div className="relative h-56 sm:h-64 overflow-hidden">
                      ...
                      {category.count && (
                        <div className="absolute bottom-4 left-4 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm px-4 py-2 rounded-full">
                          <span className="font-bold text-gray-900 dark:text-white text-sm">{category.count}+ Sabores</span>
                        </div>
                      )}
                    </div>

                    <div className="p-6">
                      <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2 font-lufga">{category.name}</h3>
                      <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">{category.subtitle}</p>
