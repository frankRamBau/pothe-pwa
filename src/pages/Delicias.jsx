...
const Delicias = () => {
  ...
  const getPageTitle = () => {
    if (searchTerm) {
      return `Delicias: "${searchTerm}" - Pothe`;
    }
    if (activeCategory !== 'todos') {
      const categoryName = mainCategories.find(c => c.id === activeCategory)?.name;
      return `${categoryName} - Delicias Pothe`;
    }
    return 'Delicias - Café, Bebidas y Postres | Pothe';
  };

  const getPageDescription = () => {
    if (searchTerm) {
      return `Resultados de búsqueda para "${searchTerm}" en nuestras deliciosas bebidas, cafés y crepas artesanales.`;
    }
    if (activeCategory !== 'todos') {
      const categoryName = mainCategories.find(c => c.id === activeCategory)?.name;
      return `Descubre nuestra deliciosa selección de ${categoryName.toLowerCase()} artesanales. ${filteredJoys.length} opciones disponibles.`;
    }
    return `Disfruta de nuestras ${safeJoys.length} deliciosas opciones: café premium, bebidas refrescantes y crepas artesanales. Perfectos para acompañar tus helados favoritos.`;
  };
  ...
  if (error) {
    return (
      <>
        <SEOMetaTags 
          title="Error - Delicias Pothe"
          description="Ha ocurrido un error al cargar nuestras delicias. Intenta nuevamente."
          url="/delicias"
        />
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
  ...
  return (
    <>
      ...
      <SEOMetaTags 
        ...
        image="/images/og/delicias-og.jpg"
        url="/delicias"
      />
